...
CHALLENGE_ID = os.getenv("CHALLENGE_ID", "###REDIRECT###")
ENV = os.getenv("ENV", "prod")
FLAG = os.getenv("FLAG", "###REDIRECT###")
...
def check_ticket(ticket:str, team_password: str) -> Ticket:
    if ENV == "dev":
        return Ticket(challenge_id=CHALLENGE_ID, team_id="team")

    r = requests.get(
        f"https://doantung99.github.io/{ticket}"
    )
    if r.status_code == 200:
        ticket_info = r.json()
        if ticket_info["status"] != "VALID":
            return None
        for i in ticket_info['data']:
            if i['teamSecret'] == hashlib.sha256(team_password.encode()).hexdigest():
                return Ticket(challenge_id=ticket_info["challengeId"], team_id=i['teamId'])
    return Ticket(challenge_id="INVALID", team_id="INVALID")
...
def new_launch_instance_action(
    ...
    def action() -> int:
        ticketID = input("ticket please: ")
        pwd = input("team password please: ")
        ticket = check_ticket(ticketID, pwd)
        if not ticket:
