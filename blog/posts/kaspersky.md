...
This challenge is a near-perfect case study in division of labour: the LLM is extraordinary at *grinding* (enumerate 88 slugs, parse dozens of PDFs, batch-submit 200 strings, reverse a SPA's fetch calls), and reliably bad at *two* things that decided the whole challenge — over-generalizing a single observation into a law, and assuming the answer's *language* from a *format* hint. My value was prompting precisely around those two weaknesses.

**Prompt the OSINT primitive, not the question.** The phrasings are designed to mislead a literal reader. Translate each into the platform/artifact it actually denotes before letting the model search. The prompts that moved things:

> "'Modified date of the file' is a metadata question, not a search question. Don't Google it — fetch the file directly and read its `Last-Modified` header and the PDF `ModDate`/XMP field. Report all three dates you find."

> "'Visited more than 50 times' is a literal check-in badge on a location-based social network, and a *room* (indoor) rules out Strava. This is Foursquare/Swarm. Find the Kaspersky HQ gym venue, and if the page 302-redirects to login, stop scraping — the data is auth-gated; we log in and read the 'Been here 50+ times' badge in Tips."

**Tell it which dead-ends to avoid, by name.** Two were predictable and worth pre-empting in the prompt:
- *Domain bias on Q3:* "The registration link is almost certainly **off-domain** — the format hint `example.com/abcxyz` means a third-party short link. Do NOT keep guessing kaspersky.com / Starmus / JunctionX paths. (And JunctionX is impossible here: that program started in 2018, the repost is Aug 2017.)"
- *Over-generalizing Q2 on Q5:* "Q2's 'data lives in curriculum PDFs' is a hypothesis for *this* question only, not a law. Before you commit to a Wayback/SOCKS5 PDF hunt for Q5, re-query the **live modern API** and prove it wrong first. Cheapest experiment first." — Had I issued this earlier, we'd have skipped the entire SOCKS5/PDF detour.

**For graded-string challenges, exploit the grader and force the source's native script.** The two prompts that actually finished it:

> "Build a replay harness: `POST /api/start`, replay the four confirmed answers to re-mint a Q5 token, then submit each Q5 candidate. The 3-wrong lockout doesn't actually fire — only HTTP 429 does, so add a 250 ms delay and retry on 429. Watch for `message: 'Completed.'`, not 'Correct.', as the success signal."

> "Stop transliterating. The Russian-locale page is the source, so the canonical answer is **Cyrillic**. Submit the raw `name` field from the `domainType=1` API verbatim — `Кибергигиена` — and put it first in the batch. The underscore format hint describes punctuation, not language."

**How I verified / caught the model's mistakes.** Three habits did all the work:
1. *Make every hypothesis cheap and falsifiable.* I never argued with a guess — I had the model fire it at the grader (or do the date arithmetic) so the *server* ruled it out. That converted "I think it's Starmus" into a one-line `Wrong answer` and moved on.
2. *Distrust derived strings; trust raw fields.* The model kept inventing romanizations; the fix was "read the `name` field, don't spell it yourself." When an authoritative source returns the exact string, submit *that*, byte-for-byte.
3. *Cheapest experiment first.* The SOCKS5/PDF marathon happened because we committed to an expensive hypothesis before testing a free one (re-query the live API). The check I now apply: "what is the laziest possible way to disprove this?" — and do that before any infrastructure work.

**Fast-path prompt recipe for next time:** *"Translate each clue into the concrete platform/artifact it denotes (metadata, archived curriculum, check-in badge, native-locale API); script the grader and replay known answers to batch-test cheaply; submit the source's raw field verbatim — in its native script, not your transliteration; and before any expensive hunt, disprove the cheap on-the-live-site hypothesis first."*
